...
Get the new files from CMS, open the "DMEPOS25_XXX.csv" file and delete the first 6 rows. Then delete column D "Juris", then delete "Ceiling" to "TN (R)", then delete "UT (NR)" to "VI (R)". This will leave you with "HCPCS	Mod	Mod2	CATG	TX (NR)	TX (R)	Description"
